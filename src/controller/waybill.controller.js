...
export const createWaybill = asyncHandler(async (req, res) => {
    ...
    if (!token) return res.status(500).json({ message: "failed to generate bluedartToken" });
    ...
    try {
        ...
        for (const product of data?.products) {
            await orderModel.findByIdAndUpdate({ _id: product?._id }, { trackDetail: trackDetail })
        }

        const resp2 = await TrackModel.create({ ...trackDetail, user_id: req?.user._id, PickupDate });

        return res.status(200).json({ message: "waybill Generated Succesfully", data: resp2 });
...
export const CancelPickup = asyncHandler(async (req, res) => {
    const token = await GenerateToken()
    const data = req.body
    if (!token) return res.status(500).json({ error: 'Failed to generate BlueDart token' });
    ...
    try {
        ...
        return res.status(error.response?.status || 500).json({ error: error.message || 'Internal Server Error' });
    }
...
export const TrackingShipment = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({ data: resp?.data, message: "Tracking detail of the product" });
